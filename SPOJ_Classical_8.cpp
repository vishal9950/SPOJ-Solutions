...
Input
There is a single positive integer T on the first line of input (equal to about 5000). It stands for the number of test cases to follow. Each test case consists of two lines. First line of each test case contains two integer numbers S and Cseparated by a single space, 1 <= S < 100, 1 <= C < 100, (S+C) <= 100. The first number, S, stands for the length of the given sequence, the second number, C is the amount of numbers you are to find to complete the sequence.
...
void next(int c,int i, int j)
{
    for(int x=0;x<s-1;x++,i=0,j++)
    {
        for(int k=0;k<s-x-1;k++,i++)
        {
            mod[i][j+1]=mod[i+1][j]-mod[i][j];
        }
    }
    for(int x=i+1,k=j;x<s+c;x++)
    {
        mod[x][k]=mod[x-1][k];
    }
    for(int k=j-1;k>=0;k--)
    {
        for(int x=i+1;x<s+c;x++)
        {
            mod[x][k]=mod[x-1][k]+mod[x-1][k+1];
        }
    }
    for(int x=s;x<s+c;x++)
        cout << mod[x][0] << " ";
